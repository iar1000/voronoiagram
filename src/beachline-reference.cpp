...
Arc* BeachLine::arcAbove(const Point& point) const
{
	...
	// go through beachline
	for (Arc* arc=_firstElement; arc; arc=arc->next()) {

		if (!arc->next()) { // if reached the last arc
			if (arc->site()->position().y()==point.y()) return 0;
			return arc;
		}

        // checks if constructing point of this arc is higher
        //  than from next arcs constructing point
        // todo: what is it used for ??
		bool left = arc->site()->position().y()>arc->next()->site()->position().y();

        // create solution set, if there is not a finite amount, try next arc
