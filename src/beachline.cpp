...
Arc* Beachline::getArcAbove(Point* p){
	...
	for (Arc* arc=m_leftmost; arc; arc=arc->next()) {
		...
		Point* vertex_para_arc = Point(arc->constructingPointX(), arc->constructingPointY() - stretch_para_arc);
        double stretch_para_next_arc = (arc->next()->constructingPointY() - directrix)/2;
		Point* vertex_para_next_arc = Point(arc->next()->constructingPointX(), arc->next()->constructingPointY() - stretch_para_next_arc);
        ...
        // two intersection points
            ...
            if(left){ intersection = new Point(x, y); }
            else { intersection = new Point(x2, y2); }
        }
        // **** create solution set *****
